...
def draw_keypoints(keypoints, img, radius = 2, color = (0,180,0), thickness=2):
    try:
        y, x, _ = img.shape
...
def draw_bbox(x_coord, y_coord, height, length, img, color = (180,0,0), thickness=2):
    try:
        y, x, _ = img.shape
...
if __name__ == "__main__":
    try:
        ...
        while success:
            ...
            x_bbox, y_bbox, height_bbox, length_bbox = compute_bbox(keypoints, threshold=threshold)
            bbox_area = height_bbox*length_bbox


            img = draw_keypoints(keypoints, img)
            img = draw_bbox(x_bbox, y_bbox, height_bbox, length_bbox, img)

            freq = keypoints[0][0][0][0].numpy()*200 +200
            
            print(freq, "    |     ",  x_bbox, y_bbox, height_bbox, length_bbox)
