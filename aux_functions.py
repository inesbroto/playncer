...
def build_pitch_grid(x_dim, y_dim, printGrid = False, num_random_points=10, min_fq=200, max_fq=800, corner_pitch_values=None):
    """
    ...
    try:
        ...
        random_pitch_values = np.random.uniform(0, 1, size=num_points)  # MIDI or Hz
        ...
        corner_pitch_values = [0.5]*4 if corner_pitch_values is None else [i/max_fq for i in corner_pitch_values] #corner_pitch_values are gonna be the mid freq value if not initialized
        ...
        interpolated_pitch = rescale_linear(interpolated_pitch, min_fq, max_fq) #hearing is between 20 and  20000
        ...
        if printGrid:
            ...
            plt.scatter(*zip(*all_points), c=[interpolated_pitch[all_points[i][1], all_points[i][0]] for i in range(len(all_points))], edgecolor="white", label="Defined Points", linewidths=0.3)
            plt.legend()
            plt.title(f"2D Pitch Mapping using {num_random_points} points")
            plt.xlabel("X")
            plt.ylabel("Y")
            plt.show()
